...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ borderBottom: '1px solid #1e1e1e', padding: '16px 0' }}>
      <div style={{ fontSize: 9, fontWeight: 500, color: '#444', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 12, fontFamily: "'DM Mono', monospace" }}>{label}</div>
      {children}
    </div>
  )
}
...
function BuilderInner() {
  ...
  const inp: React.CSSProperties = { width: '100%', padding: '8px 10px', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 3, fontSize: 12, color: '#e0e0e0', fontFamily: "'DM Sans', sans-serif", outline: 'none' }
  const hexInp: React.CSSProperties = { width: 72, padding: '4px 8px', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 3, fontSize: 11, color: '#e0e0e0', fontFamily: "'DM Mono', monospace", outline: 'none' }
  const tabBtn = (tab: typeof activeTab): React.CSSProperties => ({
    flex: 1, padding: '8px 4px', background: activeTab === tab ? '#1e1e1e' : 'transparent',
    border: 'none', borderBottom: activeTab === tab ? '1px solid #f0f0f0' : '1px solid transparent',
    color: activeTab === tab ? '#f0f0f0' : '#555', fontSize: 10, cursor: 'pointer',
    fontFamily: "'DM Mono', monospace", letterSpacing: '0.08em', textTransform: 'uppercase',
    transition: 'all 0.15s',
  })

  return (
    <div data-theme={builderDark ? 'dark' : 'light'} style={{ minHeight: '100vh', background: '#0c0c0c', display: 'flex', flexDirection: 'column', fontFamily: "'DM Mono', monospace" }}>

      {/* Header */}
      <header style={{ padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #1e1e1e', background: '#0c0c0c' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 10, color: '#555', letterSpacing: '0.1em' }}>LINKDROP</span>
          <span style={{ color: '#2a2a2a' }}>/</span>
          <span style={{ fontSize: 11, color: '#f0f0f0' }}>@{handle}</span>
          {viewCount > 0 && (
            <span style={{ fontSize: 9, color: '#555', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 3, padding: '2px 7px' }}>
      ...
      <div style={{ flex: 1, display: 'flex', gap: 0 }}>
        ...
        {/* Left panel — tabs */}
        <div style={{ width: 320, borderRight: '1px solid #1e1e1e', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>

          {/* Tabs */}
          <div style={{ display: 'flex', borderBottom: '1px solid #1e1e1e' }}>
            {(['profile', 'links', 'theme', 'settings'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} style={tabBtn(tab)}>{tab}</button>
            ))}
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 16px' }}>
            ...
            {activeTab === 'links' && (
              <>
                <Section label="Social quick-add">
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                    {SOCIAL_QUICK.map(s => (
                      ...
                      }} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 8px', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 3, fontSize: 9, color: '#888', cursor: 'pointer', fontFamily: "'DM Mono', monospace", letterSpacing: '0.05em' }}>
                ...
                <Section label={`Links · ${links.length}`}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 10 }}>
                    {links.map(link => (
                      <div key={link.id} draggable onDragStart={e => onDragStart(e, link.id)} onDragOver={e => onDragOver(e, link.id)} onDrop={e => onDrop(e, link.id)} onDragLeave={() => setDragOverId(null)}
                        style={{ background: dragOverId === link.id ? '#1e1e1e' : '#141414', border: `1px solid ${dragOverId === link.id ? '#333' : '#1e1e1e'}`, borderRadius: 3, overflow: 'hidden', transition: 'all 0.1s' }}>

                        {/* Link header row */}
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 10px' }}>
                          <span style={{ fontSize: 10, color: '#333', cursor: 'grab' }}>⠿</span>
                          {link.pinned && <span style={{ fontSize: 8, color: '#888', background: '#1e1e1e', border: '1px solid #2a2a2a', borderRadius: 2, padding: '1px 4px' }}>PIN</span>}
                          <span style={{ flex: 1, fontSize: 11, color: '#e0e0e0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{link.label}</span>
                          {clickCounts[link.id] > 0 && <span style={{ fontSize: 8, color: '#555', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 2, padding: '1px 5px', flexShrink: 0 }}>{clickCounts[link.id]} clicks</span>}
                          <button onClick={() => startEdit(link)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#444', fontSize: 10, padding: '0 2px' }}>✎</button>
                          <button onClick={() => togglePin(link.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: link.pinned ? '#f0f0f0' : '#444', fontSize: 10, padding: '0 2px' }}>📌</button>
                          <button onClick={() => removeLink(link.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#444', fontSize: 13, padding: '0 2px', lineHeight: 1 }}>×</button>
                        </div>
                  ...
                  {addingLink ? (
                    ...
                    <button onClick={() => setAddingLink(true)} style={{ width: '100%', padding: '8px', background: 'transparent', border: '1px dashed #2a2a2a', borderRadius: 3, fontSize: 9, color: '#555', cursor: 'pointer', fontFamily: "'DM Mono', monospace", letterSpacing: '0.08em' }}>
                      + ADD CUSTOM LINK
                    </button>
                  )}

                  <div style={{ marginTop: 8, textAlign: 'right' }}>
                    <button onClick={refreshClicks} style={{ fontSize: 9, color: '#444', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'DM Mono', monospace", letterSpacing: '0.06em' }}>↻ REFRESH CLICKS</button>
                  </div>
                </Section>
              </>
            )}
            ...
            {activeTab === 'settings' && (
              <>
                <Section label="Your link">
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 10px', background: '#141414', border: '1px solid #1e1e1e', borderRadius: 3 }}>
                    <span style={{ fontSize: 10, color: '#888' }}>linkdrop.ayteelabs.com/<strong style={{ color: '#f0f0f0' }}>{handle}</strong></span>
                    <button onClick={copyUrl} style={{ fontSize: 9, color: '#555', background: 'none', border: 'none', cursor: 'pointer', letterSpacing: '0.06em' }}>COPY</button>
                  </div>
                </Section>
                ...
                <Section label="Explore">
                  <a href="/explore" target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 12px', background: 'transparent', color: '#888', border: '1px solid #2a2a2a', borderRadius: 3, fontSize: 9, textDecoration: 'none', fontFamily: "'DM Mono', monospace", letterSpacing: '0.08em' }}>
                    VIEW EXPLORE PAGE →
                  </a>
                </Section>
              </>
        ...
        {/* Right panel — preview */}
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: '#0c0c0c', backgroundImage: `linear-gradient(rgba(255,255,255,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.02) 1px, transparent 1px)`, backgroundSize: '32px 32px', gap: 12 }}>
          <div style={{ fontSize: 9, color: '#333', letterSpacing: '0.12em', textTransform: 'uppercase' }}>preview</div>
